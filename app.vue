...
<template>
  <div class="common-layout">
    ...
    <el-container>
      <el-header class=" z-999 w-full" :class="{ fixed: fixed }">
        <menuHead></menuHead>
      </el-header>
      ...
      <el-footer style="padding: 0; ">
        <div v-if="route.path != '/' && route.name != 'Product-id'" style="display: flex;justify-content: center"> <button
            @click="openState = !openState" class="footerButton">
        ...
        <div v-auto-anim>
          <div v-if="openState" style="width: 100%;background-color:  #24262b">
            <div class=" footer" style="line-height: 1.5;">
              <div class="row">
                <div class="footer-col">
                  ...
                  <ul>
                    ...
                    <li>
                      <NuxtLink to="/server">工厂环境</NuxtLink>
                    </li>
                    <li>
                      <NuxtLink to="/parter">合作伙伴</NuxtLink>
                    </li>
                    <li>
                      <NuxtLink to="/callme">联系我们</NuxtLink>
                    </li>
              ...
              <div style="color: rgb(107 104 104)">Copyright © 2023 - 2028 化工有限公司 All Rights Reserved
              </div>
            </div>
          </div>
        </div>
...
const open = () => {
  ElMessageBox.alert(`${Front.value['home_modal_content'].value}`, 'Hello', {
...
if (!!time.value &&(new Date(time.value)>=new Date(todayStart) && new Date(time.value)<=new Date(todayEnd))) {
  // time.value=false
} else {
  // 更新时间值为当前时间
  open()}}
